package org.maxgamer.maxclans.util.nms.v1_5_R3;
...
public class NMSCore extends NMSDependent{
	...
	public Inventory getInventory(byte[] bytes, String name, int size){
		...
		for(int i = 0; i < list.size(); i++){
			...
			net.minecraft.server.v1_5_R3.NBTTagCompound item = (net.minecraft.server.v1_5_R3.NBTTagCompound) list.get(i);
			//The item has a tag "index", where it is stored in the chest
			int index = item.getInt("index");
			//Create an itemstack from the nbt tag
			net.minecraft.server.v1_5_R3.ItemStack is = net.minecraft.server.v1_5_R3.ItemStack.createStack(item);
	...
	public byte[] getBytes(Inventory inv) {
		//Blank NBT tag
		net.minecraft.server.v1_5_R3.NBTTagCompound c = new net.minecraft.server.v1_5_R3.NBTTagCompound();
		//Blank NBT list
		...
		for(int index = 0; index < inv.getContents().length; index++){
			...
			org.bukkit.craftbukkit.v1_5_R3.inventory.CraftItemStack cis = (org.bukkit.craftbukkit.v1_5_R3.inventory.CraftItemStack) inv.getItem(index);
			if(cis != null){ //If cis == null, no item is there, ignore it.
				//Convert it to a NMS itemstack
				net.minecraft.server.v1_5_R3.ItemStack is = org.bukkit.craftbukkit.v1_5_R3.inventory.CraftItemStack.asNMSCopy(cis);
