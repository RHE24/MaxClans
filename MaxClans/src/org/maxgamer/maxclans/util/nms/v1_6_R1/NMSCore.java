package org.maxgamer.maxclans.util.nms.v1_6_R1;
...
public class NMSCore extends NMSDependent{
	...
	public Inventory getInventory(byte[] bytes, String name, int size){
		...
		net.minecraft.server.v1_6_R1.NBTTagList list = c.getList("inventory");
		...
		for(int i = 0; i < list.size(); i++){
			...
			net.minecraft.server.v1_6_R1.NBTTagCompound item = (net.minecraft.server.v1_6_R1.NBTTagCompound) list.get(i);
			//The item has a tag "index", where it is stored in the chest
			int index = item.getInt("index");
			//Create an itemstack from the nbt tag
			net.minecraft.server.v1_6_R1.ItemStack is = net.minecraft.server.v1_6_R1.ItemStack.createStack(item);
			...
			ItemStack cis = org.bukkit.craftbukkit.v1_6_R1.inventory.CraftItemStack.asBukkitCopy(is);
	...
	public byte[] getBytes(Inventory inv) {
		...
		//Blank NBT list
		net.minecraft.server.v1_6_R1.NBTTagList list = new net.minecraft.server.v1_6_R1.NBTTagList();
		
		//Iterate over the inventory contents
		for(int index = 0; index < inv.getContents().length; index++){
			//Convert it to a craftitem stack
			org.bukkit.craftbukkit.v1_6_R1.inventory.CraftItemStack cis = (org.bukkit.craftbukkit.v1_6_R1.inventory.CraftItemStack) inv.getItem(index);
			if(cis != null){ //If cis == null, no item is there, ignore it.
				//Convert it to a NMS itemstack
				net.minecraft.server.v1_6_R1.ItemStack is = org.bukkit.craftbukkit.v1_6_R1.inventory.CraftItemStack.asNMSCopy(cis);
				...
				itemCompound.set("index", new net.minecraft.server.v1_6_R1.NBTTagInt("index", index));
		...
		byte[] bytes = net.minecraft.server.v1_6_R1.NBTCompressedStreamTools.a(c);
